<script lang="ts">
	...
	async function formatAI() {
		...
		} else if (wordCount(userPrompt) > 50) {
			errorMessage = 'Text length for AI formatting cannot exceed 50 words.';
		} else {
			errorMessage = '';
			loading = true;
			try {
...
<div class="mx-auto mt-4 text-sm text-gray-500">
	...
	<p>
		<strong>AI Fix:</strong> slower, 50 words limit, preserves paragraphs.
	</p>
</div>
